...
export default function AdminDashboard() {
  ...
  const cards = [
    ...
    { label: "Pending Orders", value: data.pending_orders, alert: data.pending_orders > 0 },
    { label: "Published Products", value: `${data.published_products} / ${data.total_products}` },
    { label: "Low Stock Variants", value: data.low_stock_variants, alert: data.low_stock_variants > 0 },
  ];
        ...
        {data.recent_orders.length === 0 ? (
          ...
          <ul className="mt-4 divide-y divide-ink/10 border-y border-ink/10">
            {data.recent_orders.map((o) => (
              <li key={o.order_number} className="flex items-center justify-between py-3 text-sm">
